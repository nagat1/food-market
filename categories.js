...
function displaycategories(data) {
  ...
  for (var i = 0; i < data.categories.length; i++) {
    box += ` <div class="col-md-3 " onclick="getcountryname('${data.categories[i].strCategory}')">
        <div
        ...
        >
          ...
          >
            <h3 class=""fs-3>
            ${data.categories[i].strCategory}</h3>
          <P>  ${data.categories[i].strCategoryDescription.split("").splice(0, 100).join("")}</P>
          </div>
        </div>
      </div>
        `
