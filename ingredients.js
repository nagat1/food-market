...
function displaycountries(data) {
  ...
  for (var i = 0; i < data.meals.length; i++) {
    box += `
      <div class="col-md-3">
        <div class="d-flex flex-column text-center"onclick="getingredientmeals('${data.meals[i].strIngredient}')">
          <i class="fa-solid fa-drumstick-bite fa-2xl" style="color: #ffffff;"></i><br>
          <h3 class="text-white fs-3">${data.meals[i].strIngredient}</h3>
          <p class="text-white fs-6 text-center">${data.meals[i].strDescription.split("").splice(0,100).join("")}</p>
        </div>
      </div>
    `;
    dataa.innerHTML = box;}
...
// ///////////second//////////////////////
async function getingredientmeals(cat) {

  let url = `https://www.themealdb.com/api/json/v1/1/filter.php?i=${cat}`;
...
function displayDetails(mealid) {
  ...
  <span class="badge bg-primary fs-5"> 
   ${mealid.meals[0].strMeasure10} ${mealid.meals[0].strIngredient10}
</span>               
  <span class="badge bg-primary fs-5"> 
   ${mealid.meals[0].strMeasure11} ${mealid.meals[0].strIngredient11}
</span>               
  <span class="badge bg-primary fs-5"> 
   ${mealid.meals[0].strMeasure12} ${mealid.meals[0].strIngredient12}
</span>               
  <span class="badge bg-primary fs-5"> 
   ${mealid.meals[0].strMeasure13} ${mealid.meals[0].strIngredient13}
</span>               
   </p>
    ...
    <button class="btn badge bg-success fs-4" onclick='${mealid.meals[0].strYoutube}'>YouTube</button>
